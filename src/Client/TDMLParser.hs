...
{-# HLINT ignore "Use <$>" #-}
module Client.TDMLParser where
import qualified DataModel as DM
import Client.TDMLLexer
...
data MovementParams =
     ...
     | NotesParam String
     | LabelsParam [DM.Label]
     | TargetsParam [DM.Target]
     | IterationParam DM.MovementIteration
     | ScalersParam [DM.Scaler]
     | MeasuresParam [DM.Measure]
     | SubmovementsParam [[MovementParams]]
     deriving (Show)       

fromMovementParams :: [MovementParams] -> Maybe DM.Movement
fromMovementParams ps = do
     ...
     return (DM.Movement description notes labels targets iteration scalers measures submovements)
     where
...
movementEntry = do
     ...
     colonEntryParser movement $ do
          description <- string
          newline
          ps <- movementParamList [DescriptionParam description]
          let maybeMovement = fromMovementParams ps
          case maybeMovement of
               Just m -> return m
               Nothing -> parseError "Incomplete movement"

movementParamList :: [MovementParams] -> TokenParser [MovementParams]
...
movementParam :: TokenParser MovementParams
movementParam = (TokenNotes,notesEntry <&> NotesParam) <|>
                (TokenLabels,labelsEntry <&> LabelsParam) <|>
                (TokenTargets,targetsEntry <&> TargetsParam) <|>
                (TokenIteration,iterationEntry <&> IterationParam) <|>
                (TokenScalers,scalersEntry <&> ScalersParam) <|>
                (TokenMeasures,measuresEntry <&> MeasuresParam) <|>
                (TokenSubmovements,submovementsEntry <&> SubmovementsParam) <|>
                parseError "Incomplete movement specification."

submovementsEntry :: TokenParser [[MovementParams]]
